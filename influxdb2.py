...
influxdbUrl = "https://eu-central-1-1.aws.cloud2.influxdata.com/api/v2/write?org=<ORGANIZATION>&bucket=<BUCKET>&precision=s"
    ...
    while True:
        try:
            requests.request(
                method="POST",
                url=influxdbUrl,
                data=f"picoTemp,sensor_id=CPU temperature={temp} {response['unixtime']}",
                headers={
                    "Authorization": f"Token {influxdbToken}",
                    "Content-Type": "text/plain; charset=utf-8",
                    "Accept": "application/json",
                },
            )
            # print(inflresp.content)
